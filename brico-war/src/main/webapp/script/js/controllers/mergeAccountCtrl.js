					...
					if($scope.state == 'N'){
						...
						if($scope.mergeUser.password0 != null && $scope.mergeUser.password0 !== '') {
							...
							if($scope.mergeUser.password0 !== $rootScope.user.password){
								...
								// Vérification que l'user avait bien rempli les champs password et password2 avec le bon psw pour prendre en compte la modif suivante:
								update = false;
								$scope.alerts = utilSrv.alertIt('danger', 'Vous n\'avez saisi aucun nouveau mot de passe.');
							}else{
								// Si tout est ok: affectation du nv psw à l'user qu'on enverra ensuite au serveur:
